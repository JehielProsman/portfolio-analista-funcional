# 👋 Jehiel Alejandro Prosman

**Analista Funcional & Tester Manual | QA/UAT | SQL | Scrum | Nexo IT–Negocio**

📍 Rosario, Santa Fe, Argentina  
📧 jehielprosman@gmail.com  
🔗 [LinkedIn](https://www.linkedin.com/in/jehielprosman)  
📄 CV disponible a solicitud — jehielprosman@gmail.com

---

## 🧑‍💼 Sobre mí

Analista Funcional y Tester QA Manual con casi 3 años de experiencia en relevamiento 
de requerimientos, testing QA/UAT y documentación funcional, más casi 8 años en 
administración comercial, créditos, cobranzas y logística en el sector agroindustrial.

Perfil híbrido IT–negocio: entiendo el negocio desde adentro y lo traduco en 
requerimientos accionables para el equipo de desarrollo.

En mi equipo me llaman **"el rompedor"** — porque encuentro los bugs antes de que 
lleguen a producción.

Finalizando la carrera de **Analista de Sistemas de Información** (2 materias restantes).

---

## 🛠️ Competencias

| Área | Herramientas |
|---|---|
| Análisis Funcional / QA | Relevamiento, HU, criterios de aceptación, casos de uso, QA/UAT |
| Gestión de incidencias | Jira, Zephyr |
| Datos | SQL, MySQL |
| Metodologías | Scrum, Sprints |
| ERP / Sistemas | SAP SD, Softland, Synagro, Salesforce |
| IA Aplicada | ChatGPT, Claude, Gemini |
| Desarrollo | PHP, Python, Java, C#, HTML, .NET |

---

## 📂 Contenido de este portfolio

| Carpeta | Descripción |
|---|---|
| [`casos-de-prueba`](./casos-de-prueba) | Ejemplos de casos de prueba documentados |
| [`historias-de-usuario`](./historias-de-usuario) | Templates de historias de usuario y criterios de aceptación |
| [`sql-queries`](./sql-queries) | Consultas SQL de validación de datos |

---

## 📬 Contacto

Abierto a oportunidades en IT, análisis funcional, QA y roles híbridos IT–negocio.  
Disponibilidad para trasladarse.

✉️ jehielprosman@gmail.com
